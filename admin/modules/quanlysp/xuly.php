...
if (isset($_POST['themsanpham'])) {
    ...
    if ($hinhanh != '') {
        ...
        //xoa hinh anh cu
        $sql = "SELECT * FROM tbl_sanpham WHERE id_sanpham = '$_GET[idsanpham]' LIMIT 1"; //lấy sp chứa hình ảnh mình muốn xóa
    ...
    $id = $_GET['idsanpham'];
    $sql = "SELECT * FROM tbl_sanpham WHERE id_sanpham = '$id' LIMIT 1"; //lấy sp chứa hình ảnh mình muốn xóa
    $query = mysqli_query($mysqli, $sql);
    while ($row = mysqli_fetch_array($query)) {
        unlink('uploads/' . $row['hinhanh']);
    }
